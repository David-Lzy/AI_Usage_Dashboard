...
- adapter diagnostic body localization remains blocked by [I18n_Adapter_Diagnostic_Reason_Code_Plan.md](./I18n_Adapter_Diagnostic_Reason_Code_Plan.md); `Phase 184` defines the typed reason-code plan, `Phase 185` ships the type-only additive model, and `Phase 186` populates Cursor source-selection/fallback diagnostics while keeping Codex source-selection/fallback population as the next safe step
