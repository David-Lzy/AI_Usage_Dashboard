...
- runtime locale architecture now covers 14 locales; `zh-CN` has broad reviewed non-English runtime copy, `zh-TW`, `ja`, `ko`, `es-419`, `pt-BR`, `fr`, `de`, `it`, and `ru` have first shell-level runtime pilots, and other non-English locales currently rely on English fallback for deeper runtime copy until a translation-review phase replaces those entries
