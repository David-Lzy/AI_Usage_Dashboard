...
- adapter diagnostic body localization remains blocked by [I18n_Adapter_Diagnostic_Reason_Code_Plan.md](./I18n_Adapter_Diagnostic_Reason_Code_Plan.md); `Phase 184` defines the typed reason-code plan, `Phase 185` ships the type-only additive model, `Phase 186` populates Cursor source-selection/fallback diagnostics, `Phase 187` populates Codex source-selection/fallback diagnostics, `Phase 188` populates Cursor/Codex credential plus host-access diagnostics, and `Phase 189` populates Cursor/Codex page-session diagnostics while keeping usage-threshold plus policy-only diagnostic typing as the next safe step
