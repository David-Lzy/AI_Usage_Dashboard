...
Complete the remaining popup structured-copy translation after featured provider copy is covered.

## Scope

- Translate `actionSection`, `surfaceRoles`, and `aria` in `src/shared/popup-localized-copy.ts`.
- Preserve glossary terms fixed in `Phase 392.1`.
- Add focused tests proving every non-English locale has translated representative action-section, surface-role, and aria copy.
