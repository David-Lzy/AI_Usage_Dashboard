# Phase 374 - Japanese Runtime Shell Pilot
...
Add the next focused runtime shell translation pilot for `ja`, so Japanese users no longer see English fallback across the first dashboard, popup, and Settings shell surfaces after selecting the language.

## Scope

- Add `ja` runtime message overrides for:
...
- Add focused runtime i18n tests for the first `ja` shell slice.
...
- `createRuntimeI18n("ja")` returns Japanese strings for the visible dashboard, popup, and Settings shell labels covered by this slice.
...
- Added the first `ja` runtime shell override set in `src/shared/i18n.ts`.
- Added focused `ja` runtime message assertions in `src/shared/i18n.test.ts`.
- Updated maintained localization docs and phase indexes to record Japanese as the next runtime shell pilot locale after `zh-TW`.
...
- Future translation-review phases should extend `ja` into structured helper copy before treating it as a broad reviewed runtime locale.
