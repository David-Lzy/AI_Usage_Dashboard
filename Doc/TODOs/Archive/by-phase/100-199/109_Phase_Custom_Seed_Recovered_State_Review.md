...
Prove the shipped custom-seed path stays coherent when a seeded session-page warning state recovers back to `ready`.
...
- the product could prove degraded or healthy states in isolation
- but it had not yet proven that one warning state could recover back to a healthy state without losing the saved custom-seed contract

This phase existed to close that gap with the next smallest honest contract:

- one seeded recovered-state review
- using the shipped `Cursor` and `Codex` session-page paths
- proving `host_access_missing` warning surfaces recover to neutral healthy surfaces under the same saved custom seed

## Exit Criteria

- one repeatable seeded recovered-state custom-seed review script exists
- the review proves seeded `host_access_missing` warning surfaces exist for `Cursor` and `Codex`
- the review proves those same providers recover to neutral healthy surfaces under the same saved custom seed
- the review proves popup snapshot and featured-provider surfaces recover coherently
