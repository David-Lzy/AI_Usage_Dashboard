...
Current post-`Phase 392.2` execution priority:
...
5. `P1` - continue the split popup localization queue after `Phase 392.2`: active `Phase 392.2.1` completes featured-section and featured-card copy across 14 locales; queued `Phase 392.2.2` completes action-section, surface-role, and aria popup copy; `Phase 393` expands Settings and provider-detail runtime copy; `Phase 394` audits maintenance hotspots; `Phase 395` splits runtime message catalog modules without changing behavior.
...
17. `P2` - keep the `Phase 367` through `Phase 392.1` localization expansion stable: Settings language options must keep coming from the 14-locale registry, Arabic must resolve `rtl`, manifest `_locales` catalogs must keep the stable manifest ids, English fallback text in RTL surfaces must keep natural punctuation order, RDP locale smoke captures must validate `--locale` against the 14 runtime tags, `npm run i18n:check` must keep runtime metadata, Chrome catalog directories, RDP locale helpers, and the store listing draft aligned, runtime message catalogs must preserve the public `src/shared/runtime-message-catalogs.ts` export path even if `Phase 395` later splits internal modules, every non-English locale in the 14-locale set must keep its first runtime shell slice plus the focused coverage guard, popup first-run guidance/setup/snapshot/header copy must keep explicit 14-locale coverage, and extension HTML shells must keep notranslate markers so browser translation overlays do not pollute localized screenshots.
