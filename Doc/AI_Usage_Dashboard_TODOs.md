...
Current post-`Phase 420` execution priority:
...
5. `P1` - continue the numbered queue after `Phase 420`: active `Phase 421` covers the interaction-audit surface-definition display/source split.
...
17. `P2` - keep the `Phase 367` through `Phase 420` localization expansion stable: Settings language options must keep coming from the 14-locale registry, Arabic must resolve `rtl`, manifest `_locales` catalogs must keep the stable manifest ids, English fallback text in RTL surfaces must keep natural punctuation order, RDP locale smoke captures must validate `--locale` against the 14 runtime tags, `npm run i18n:check` must keep runtime metadata, Chrome catalog directories, RDP locale helpers, and the store listing draft aligned, runtime message catalogs must preserve the public `src/shared/runtime-message-catalogs.ts` export path while internal data lives under `src/shared/runtime-message-catalog-data/`, every non-English locale in the 14-locale set must keep its first runtime shell slice plus the focused coverage guard, popup first-run guidance/setup/snapshot/header, featured provider, action-section, surface-role, and aria copy must keep explicit 14-locale coverage, Settings core, credential, source-card, permission, Provider Detail, and provider-source display copy must keep explicit 14-locale coverage, extension HTML shells must keep notranslate markers so browser translation overlays do not pollute localized screenshots, the diagnostic presentation inventory must keep raw diagnostic evidence outside translation scope, warning/source/adapter diagnostic presentation must keep explicit 14-locale coverage without replacing raw diagnostic bodies, the localized operator/store helper RDP visual QA package must remain the current representative evidence for `en`, `zh-CN`, `ja`, `de`, and `ar`, the localization copy chunk-size audit decision must stay aligned with future route-loading changes, the interaction-audit presentation/export split must keep signoff exports, handoff drafts, request ids, route ids, preset ids, command text, and manual-check evidence outside localization, store-helper error wrappers must keep raw invalid-preset, malformed-seed, and native popup probe error text embedded exactly inside localized display messages, interaction-audit Review Queue localization must keep queue status enums, surface titles, signoff exports, and generated handoff drafts unchanged, interaction-audit Surface Card localization must keep surface definitions, manual-check evidence, frame-action raw messages, iframe identity, signoff exports, and generated handoff drafts unchanged, interaction-audit Workspace Controls localization must keep pasted JSON, parsed import errors, signoff export JSON, generated Markdown drafts, filenames, MIME types, storage keys, request binding, and request revision formatting unchanged, interaction-audit Request Scope command heading localization must keep generated command text, request ids, request revisions, archive path examples, input filename examples, signoff export JSON, generated Markdown drafts, filenames, and MIME types unchanged, interaction-audit Handoff Summary localization must keep handoff draft content, generated bundle command text, operator notes, surface ids, surface titles, manual-check evidence, filenames, MIME types, and signoff export schemas unchanged, the interaction-audit frame-result typed split must keep raw diagnostic details separate from stable result codes, frame-result 14-locale display copy must keep raw selector/preset diagnostics untranslated and visible, the remaining-copy inventory must keep route feedback/accessibility labels, import-error presentation, and surface-definition display/source split as separate implementation tracks, route feedback/accessibility localization must keep dynamic surface titles source-bound until the later surface-definition split, and typed import-error presentation must keep pasted JSON plus parsed payload fields raw.
