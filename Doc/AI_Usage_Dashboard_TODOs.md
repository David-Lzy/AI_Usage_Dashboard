...
Current post-`Phase 392` execution priority:
...
5. `P1` - continue the split popup localization queue after `Phase 392`: active `Phase 392.1` expands popup first-run guidance, setup coverage, snapshot status, and header copy across 14 locales; `Phase 392.2` completes featured-card, action-section, surface-role, and aria popup copy; `Phase 393` expands Settings and provider-detail runtime copy; `Phase 394` audits maintenance hotspots; `Phase 395` splits runtime message catalog modules without changing behavior.
