...
Current post-`Phase 508` execution priority:
...
2. `P0` - treat `release/ai-usage-dashboard-0.1.0-rc.23.zip` as the latest packaged follow-up candidate. Current source is ahead of that package through `Phase 508`, which moves provider runtime identity to source-level entries, fixes long-lived-profile legacy-provider migration, keeps popup provider cards unbounded while quota-threshold warnings remain user status rather than product/setup attention, polishes the Quick Setup team/API provider toggle, compacts popup progress labels while preserving reset timing, and tightens Claude Team all-model weekly labels; this source boundary has not been packaged yet.
...
- treat `rc.13` as the current submitted store-review boundary, and `rc.23` as the latest packaged follow-up candidate through the Phase 502 package boundary; current source is ahead through `Phase 508` and has not been packaged, while `rc.22`, `rc.21`, `rc.20`, `rc.19`, `rc.18`, `rc.17`, `rc.16`, `rc.15`, `rc.14`, `rc.12`, and `rc.11` remain historical evidence
...
- `Phase 508` tightened Claude Team all-model weekly popup labels and shortened weekday reset text; no post-Phase-508 package has been cut yet
