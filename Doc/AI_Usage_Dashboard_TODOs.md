...
Current post-`Phase 380` execution priority:
...
5. `P2` - no numbered phase is currently queued after `Phase 380`. If continuing, review the project and create a new small TODO before changing behavior.
...
17. `P2` - keep the `Phase 367` through `Phase 380` localization expansion stable: Settings language options must keep coming from the 14-locale registry, Arabic must resolve `rtl`, manifest `_locales` catalogs must keep the stable manifest ids, English fallback text in RTL surfaces must keep natural punctuation order, RDP locale smoke captures must validate `--locale` against the 14 runtime tags, `npm run i18n:check` must keep runtime metadata, Chrome catalog directories, RDP locale helpers, and the store listing draft aligned, runtime message catalogs must stay in `src/shared/runtime-message-catalogs.ts`, `zh-TW`, `ja`, `ko`, `es-419`, `pt-BR`, `fr`, and `de` must keep their first runtime shell slices, and non-reviewed runtime locales may fall back to English rather than translating raw evidence fields.
