...
Current post-`Phase 368` execution priority:
...
5. `P2` - no numbered phase is currently queued after `Phase 368`. If continuing, review the project and create a new small TODO before changing behavior.
...
17. `P2` - keep the `Phase 367` and `Phase 368` localization expansion stable: Settings language options must keep coming from the 14-locale registry, Arabic must resolve `rtl`, manifest `_locales` catalogs must keep the stable manifest ids, English fallback text in RTL surfaces must keep natural punctuation order, and non-reviewed runtime locales may fall back to English rather than translating raw evidence fields.
