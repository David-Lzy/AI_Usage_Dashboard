...
Current post-`Phase 510` execution priority:
...
2. `P0` - treat `release/ai-usage-dashboard-0.1.0-rc.23.zip` as the latest packaged follow-up candidate. Current source is ahead of that package through `Phase 510`, which moves provider runtime identity to source-level entries, fixes long-lived-profile legacy-provider migration, keeps popup provider cards unbounded while quota-threshold warnings remain user status rather than product/setup attention, polishes the Quick Setup team/API provider toggle, compacts popup progress labels while preserving reset timing, tightens Claude Team all-model weekly labels, moves popup provider actions into the card header with persistent hide plus undo, and compacts popup status chips to accessible icons; this source boundary has not been packaged yet.
...
5. `P1` - no numbered phase is currently queued after `Phase 510`. If a resubmission is needed, decide whether to package a post-Phase-510 candidate instead of assuming RC23 contains the source-level provider-entry migration and popup/provider Settings polish. [Phase 496](./TODOs/Archive/by-phase/400-499/496_Phase_Provider_Setup_Display_Product_Contract.md) defines the product contract, [Phase 497](./TODOs/Archive/by-phase/400-499/497_Phase_Quick_Setup_Source_Mode_Cards.md) keeps source-mode paths visible in Quick Setup, [Phase 498](./TODOs/Archive/by-phase/400-499/498_Phase_Dashboard_Display_Eligibility_Model.md) owns display eligibility, [Phase 499](./TODOs/Archive/by-phase/400-499/499_Phase_Surface_Order_Eligibility_Alignment.md) aligns visible provider ordering, [Phase 500](./TODOs/Archive/by-phase/500-599/500_Phase_Quota_Item_Eligibility_And_Popup_Order_QA.md) aligns quota item eligibility, [Phase 501](./TODOs/Archive/by-phase/500-599/501_Phase_Provider_Setup_Display_Model_Closeout.md) closes the provider setup/display queue, [Phase 502](./TODOs/Archive/by-phase/500-599/502_Phase_RC23_Provider_Display_Cleanup_Packaging.md) packages RC23, [Phase 503](./TODOs/Archive/by-phase/500-599/503_Phase_Source_Level_Provider_Entry_Model.md) implements source-level provider entries, [Phase 504](./TODOs/Archive/by-phase/500-599/504_Phase_Legacy_Source_Entry_Error_Fix.md) fixes long-lived-profile legacy entry cleanup, [Phase 505](./TODOs/Archive/by-phase/500-599/505_Phase_Popup_Provider_Parity_And_Quota_Warning_Fix.md) removes the popup three-card cap while keeping quota warnings out of product/setup attention, [Phase 506](./TODOs/Archive/by-phase/500-599/506_Phase_Quick_Setup_Team_Api_Toggle_Button_Polish.md) makes the Quick Setup team/API provider toggle a header-aligned outlined button, [Phase 507](./TODOs/Archive/by-phase/500-599/507_Phase_Popup_Compact_Progress_Reset_Labels.md) makes popup progress labels compact while preserving visible reset timing, [Phase 508](./TODOs/Archive/by-phase/500-599/508_Phase_Claude_Team_Popup_Compact_Label_Polish.md) tightens Claude Team all-model weekly labels, [Phase 509](./TODOs/Archive/by-phase/500-599/509_Phase_Popup_Header_Actions_And_Hide_Undo.md) moves popup provider actions into the header with persistent hide plus undo, and [Phase 510](./TODOs/Archive/by-phase/500-599/510_Phase_Popup_Compact_Status_Icon_Header_Fit.md) keeps popup card headers single-line by rendering status as compact accessible icons.
...
17. `P2` - keep the `Phase 367` through `Phase 421` localization expansion stable: Settings language options must keep coming from the 14-locale registry, Arabic must resolve `rtl`, manifest `_locales` catalogs must keep the stable manifest ids, English fallback text in RTL surfaces must keep natural punctuation order, RDP locale smoke captures must validate `--locale` against the 14 runtime tags, `npm run i18n:check` must keep runtime metadata, Chrome catalog directories, RDP locale helpers, and the store listing draft aligned, runtime message catalogs must preserve the public `src/shared/runtime-message-catalogs.ts` export path while internal data lives under `src/shared/runtime-message-catalog-data/`, every non-English locale in the 14-locale set must keep its first runtime shell slice plus the focused coverage guard, popup first-run guidance/setup/snapshot/header, featured provider, action-section, surface-role, and aria copy must keep explicit 14-locale coverage, Settings core, credential, source-card, permission, Provider Detail, and provider-source display copy must keep explicit 14-locale coverage, extension HTML shells must keep notranslate markers so browser translation overlays do not pollute localized screenshots, the diagnostic presentation inventory must keep raw diagnostic evidence outside translation scope, warning/source/adapter diagnostic presentation must keep explicit 14-locale coverage without replacing raw diagnostic bodies, the localized operator/store helper RDP visual QA package must remain the current representative evidence for `en`, `zh-CN`, `ja`, `de`, and `ar`, the localization copy chunk-size audit decision must stay aligned with future route-loading changes, the interaction-audit presentation/export split must keep signoff exports, handoff drafts, request ids, route ids, preset ids, command text, and manual-check evidence outside localization, store-helper error wrappers must keep raw invalid-preset, malformed-seed, and native popup probe error text embedded exactly inside localized display messages, interaction-audit Review Queue, Surface Card, Workspace Controls, Request Scope command heading, Handoff Summary, frame-result, route-feedback/accessibility, import-error, and surface-definition display localization must keep source ids, route paths, iframe sources, preset ids, data attributes, signoff exports, generated Markdown drafts, filenames, MIME types, storage keys, request binding, request revision formatting, raw selector/preset diagnostics, pasted JSON, parsed payload fields, and export evidence unchanged.
...
- treat `rc.13` as the current submitted store-review boundary, and `rc.23` as the latest packaged follow-up candidate through the Phase 502 package boundary; current source is ahead through `Phase 510` and has not been packaged, while `rc.22`, `rc.21`, `rc.20`, `rc.19`, `rc.18`, `rc.17`, `rc.16`, `rc.15`, `rc.14`, `rc.12`, and `rc.11` remain historical evidence
...
- `Phase 425` added shared provider progress item inventory for primary quota, usage-window, and usage-balance items while keeping usage facts, raw provider evidence, diagnostics, and archive/export payloads outside progress rendering
- `Phase 426` added Settings quota item visibility and order controls for each provider across popup, sidebar, and full-page tab surfaces while leaving runtime progress rendering unchanged until the next phase consumes those preferences
...
- `Phase 435` completed circular progress ring value and geometry polish so ring centers now show numeric-only percent labels, full remaining/used semantics remain in aria text and line rows, and soft/gauge grooves share SVG geometry with the foreground arc
- `Phase 436` completed disclosure chevron state animation by making Quick Setup more-provider details use the same open-state rotation as source-card details, while preserving custom select and editable combobox arrow behavior with reduced-motion handling
- `Phase 437` completed Chrome extension error triage by identifying the Chrome Extensions `Errors` badge as stale Vite dev-server CORS records from an older dev-mode sidepanel run, verifying current `dist/` has no Vite dev-client references, rebuilding/reloading the unpacked extension, and clearing the stale Chrome error log
- `Phase 438` completed Settings quota-item 14-locale localization by moving quota/progress item headings, helper copy, empty states, surface labels, count chips, kind/availability labels, shown/hidden labels, aria labels, move buttons, and all-hidden fallback copy into the maintained Settings localized-copy path
...
- `Phase 458` made remaining-color-band color dropdowns show one visible value, with localized recommended-color names or custom hex labels as appropriate, and compacted the band controls into inline field groups while preserving storage, validation, warning thresholds, and provider quota semantics
- `Phase 459` promoted application language and theme mode into Settings overview and removed their duplicate Appearance & Sync controls while preserving locale normalization, runtime direction, theme storage, provider settings, credentials, permissions, and source preferences
...
- `Phase 469` packaged the Phase 468 source boundary as `0.1.0-rc.21`, generated `release/ai-usage-dashboard-0.1.0-rc.21.zip`, recorded SHA256 `edcd6546695b89b70a271919a4531c19053216301affbc951eb98569f4aa4079`, and preserved RC13 as the submitted Chrome Web Store review boundary
- `Phase 470` made popup Settings actions open the full-page Settings tab, added best-effort side-panel close during tab opens, and added full-page tab to sidebar top-bar switching while preserving provider behavior and release packaging
...
- `Phase 503` implemented source-level provider entries, migrating brand-level provider settings/secrets/order preferences and aligning Quick Setup plus Provider Display with display-enabled source entries
- `Phase 504` fixed the long-lived Chrome profile migration path so legacy brand-level provider ids are not appended back as runtime extras after source-level normalization
- `Phase 505` removed the popup featured-provider three-card cap and stopped classifying quota-threshold warnings as product/setup attention, so popup cards now keep all visible providers in popup order while low-quota warnings stay visible to users
- `Phase 506` moved the Quick Setup team/API provider toggle into the section header action slot and changed it to an outlined Material-style button
- `Phase 507` made toolbar popup progress labels compact and folded reset timing into the visible popup label for line and circular progress styles
...
- `Phase 510` keeps popup provider card headers single-line first by rendering card-local status as compact accessible icon chips; no post-Phase-510 package has been cut yet
