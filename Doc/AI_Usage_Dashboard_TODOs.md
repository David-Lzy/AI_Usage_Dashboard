...
Current post-`Phase 408` execution priority:
...
5. `P1` - continue the numbered queue after `Phase 408`: active `Phase 409` scopes interaction-audit consumer-copy presentation, then queued `Phase 410` scopes store-helper error presentation.
...
17. `P2` - keep the `Phase 367` through `Phase 408` localization expansion stable: Settings language options must keep coming from the 14-locale registry, Arabic must resolve `rtl`, manifest `_locales` catalogs must keep the stable manifest ids, English fallback text in RTL surfaces must keep natural punctuation order, RDP locale smoke captures must validate `--locale` against the 14 runtime tags, `npm run i18n:check` must keep runtime metadata, Chrome catalog directories, RDP locale helpers, and the store listing draft aligned, runtime message catalogs must preserve the public `src/shared/runtime-message-catalogs.ts` export path while internal data lives under `src/shared/runtime-message-catalog-data/`, every non-English locale in the 14-locale set must keep its first runtime shell slice plus the focused coverage guard, popup first-run guidance/setup/snapshot/header, featured provider, action-section, surface-role, and aria copy must keep explicit 14-locale coverage, Settings core, credential, source-card, permission, Provider Detail, and provider-source display copy must keep explicit 14-locale coverage, extension HTML shells must keep notranslate markers so browser translation overlays do not pollute localized screenshots, the diagnostic presentation inventory must keep raw diagnostic evidence outside translation scope, warning/source/adapter diagnostic presentation must keep explicit 14-locale coverage without replacing raw diagnostic bodies, the localized operator/store helper RDP visual QA package must remain the current representative evidence for `en`, `zh-CN`, `ja`, `de`, and `ar`, and the localization copy chunk-size audit decision must stay aligned with future route-loading changes.
