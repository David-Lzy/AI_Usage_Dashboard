...
Current post-`Phase 387` execution priority:
...
5. `P0` - run the new numbered queue after `Phase 387` in order: active `Phase 388` aligns current-truth documentation drift; queued `Phase 389` adds a Markdown link guard; `Phase 390` records the release-check baseline; `Phase 391` inventories deeper 14-locale runtime copy; `Phase 392` and `Phase 393` expand popup, Settings, and provider-detail runtime copy; `Phase 394` audits maintenance hotspots; `Phase 395` splits runtime message catalog modules without changing behavior.
...
17. `P2` - keep the `Phase 367` through `Phase 387` localization expansion stable: Settings language options must keep coming from the 14-locale registry, Arabic must resolve `rtl`, manifest `_locales` catalogs must keep the stable manifest ids, English fallback text in RTL surfaces must keep natural punctuation order, RDP locale smoke captures must validate `--locale` against the 14 runtime tags, `npm run i18n:check` must keep runtime metadata, Chrome catalog directories, RDP locale helpers, and the store listing draft aligned, runtime message catalogs must preserve the public `src/shared/runtime-message-catalogs.ts` export path even if `Phase 395` later splits internal modules, every non-English locale in the 14-locale set must keep its first runtime shell slice plus the focused coverage guard, and extension HTML shells must keep notranslate markers so browser translation overlays do not pollute localized screenshots.
