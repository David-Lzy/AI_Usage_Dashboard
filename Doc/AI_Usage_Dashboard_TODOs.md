...
Current post-`Phase 473` execution priority:
...
2. `P0` - treat `release/ai-usage-dashboard-0.1.0-rc.21.zip` as the latest packaged follow-up candidate. It includes the previous `rc.20` follow-up work plus `Phase 468` Settings tooltip and form-control polish; current source is ahead through `Phase 473` toolbar-icon preferences, so package a fresh RC before resubmission if that change should ship.
...
5. `P1` - no numbered phase is currently queued after `Phase 473`; start the next change from a new scoped TODO so the RC21 package boundary remains stable.
...
- treat `rc.13` as the current submitted store-review boundary, and `rc.21` as the latest packaged follow-up candidate before the current Phase 473 source delta; `rc.20`, `rc.19`, `rc.18`, `rc.17`, `rc.16`, `rc.15`, `rc.14`, `rc.12`, and `rc.11` remain historical evidence
