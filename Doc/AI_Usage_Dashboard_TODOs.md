...
Current post-`Phase 393.3` execution priority:
...
5. `P1` - continue the localization and maintenance queue after the `Phase 393.3` split: active `Phase 393.3.1` expands Provider Detail copy; `Phase 393.3.2` expands provider-source display wrapper copy; `Phase 394` audits maintenance hotspots; `Phase 395` splits runtime message catalog modules without changing behavior.
...
17. `P2` - keep the `Phase 367` through `Phase 393.2.2` localization expansion stable: Settings language options must keep coming from the 14-locale registry, Arabic must resolve `rtl`, manifest `_locales` catalogs must keep the stable manifest ids, English fallback text in RTL surfaces must keep natural punctuation order, RDP locale smoke captures must validate `--locale` against the 14 runtime tags, `npm run i18n:check` must keep runtime metadata, Chrome catalog directories, RDP locale helpers, and the store listing draft aligned, runtime message catalogs must preserve the public `src/shared/runtime-message-catalogs.ts` export path even if `Phase 395` later splits internal modules, every non-English locale in the 14-locale set must keep its first runtime shell slice plus the focused coverage guard, popup first-run guidance/setup/snapshot/header, featured provider, action-section, surface-role, and aria copy must keep explicit 14-locale coverage, Settings core, credential, source-card, and permission copy must keep explicit 14-locale coverage, and extension HTML shells must keep notranslate markers so browser translation overlays do not pollute localized screenshots.
