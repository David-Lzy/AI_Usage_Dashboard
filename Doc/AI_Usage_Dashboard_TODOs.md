...
Current post-`Phase 500` execution priority:
...
2. `P0` - treat `release/ai-usage-dashboard-0.1.0-rc.22.zip` as the latest packaged follow-up candidate. It includes post-`rc.21` UI polish, toolbar badge/icon preferences, configuration backup and Chrome Sync, pre-store maintenance cleanup, bundle split, and CSS consolidation through `Phase 487`; current source is ahead through `Phase 500` with AGPL-3.0 open-source licensing, digital fingerprint embedding, the popup provider-visibility fix, public repository readiness docs, six-locale store listing copy, refreshed public-readiness screenshot evidence, the provider setup/display product contract, Quick Setup source-mode cards, shared provider display eligibility, visible-provider surface order alignment, and quota item eligibility alignment.
...
- treat `rc.13` as the current submitted store-review boundary, and `rc.22` as the latest packaged follow-up candidate through the Phase 487 package boundary; current source is ahead through Phase 500, with Phase 501 active for provider setup/display model closeout, while `rc.21`, `rc.20`, `rc.19`, `rc.18`, `rc.17`, `rc.16`, `rc.15`, `rc.14`, `rc.12`, and `rc.11` remain historical evidence
