...
Current post-`Phase 422` execution priority:
...
5. `P1` - continue the `Phase 423` through `Phase 432` UI preference queue: consume the display preference architecture for independent provider order per popup/sidebar/full-page surface, then add per-surface quota item visibility/order, improved progress ring styles, Settings provider carousel migration, and final visual/doc closeout.
