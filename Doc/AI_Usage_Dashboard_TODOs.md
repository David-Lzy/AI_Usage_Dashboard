...
Current post-`Phase 499` execution priority:
...
2. `P0` - treat `release/ai-usage-dashboard-0.1.0-rc.22.zip` as the latest packaged follow-up candidate. It includes post-`rc.21` UI polish, toolbar badge/icon preferences, configuration backup and Chrome Sync, pre-store maintenance cleanup, bundle split, and CSS consolidation through `Phase 487`; current source is ahead through `Phase 499` with AGPL-3.0 open-source licensing, digital fingerprint embedding, the popup provider-visibility fix, public repository readiness docs, six-locale store listing copy, refreshed public-readiness screenshot evidence, the provider setup/display product contract, Quick Setup source-mode cards, shared provider display eligibility, and visible-provider surface order alignment.
