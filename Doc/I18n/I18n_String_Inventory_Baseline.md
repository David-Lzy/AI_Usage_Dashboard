...
- `zh-CN` has broad reviewed non-English runtime copy in the current source; every other non-English locale in the 14-locale set now has a first runtime shell pilot for dashboard, popup, Settings, common actions, and theme-toggle labels; `Phase 392.1` adds explicit 14-locale popup first-run guidance, setup coverage, snapshot status, and header copy; `Phase 392.3` adds explicit 14-locale popup featured-section and featured-card copy; `Phase 392.4` completes popup action-section, surface-role, and aria copy; `Phase 393.1` completes Settings core layout, Quick Setup, preferences, and theme-customization copy; `Phase 393.2.1` completes Settings credential copy; `Phase 393.2.2` completes Settings source/permission copy; `Phase 393.3.1` completes Provider Detail copy; `Phase 393.3.2` completes provider-source display copy; `Phase 398` completes warning diagnostic presentation copy; `Phase 399` completes source-selection and source-fallback diagnostic presentation copy; `Phase 400` completes adapter-error diagnostic presentation copy; `Phase 438` completes Settings quota/progress item control copy; `Phase 465` completes Settings UI font-family preference copy; deeper runtime copy outside those buckets may still fall back to English until a future translation-review slice replaces those strings
...
1. operator-workspace and store-helper follow-up only after the end-user surfaces are stable
...
- `zh_TW`, `ja`, `ko`, `es_419`, `pt_BR`, `fr`, `de`, `it`, `ru`, `ar`, `hi`, and `id` currently cover Chrome manifest surfaces, Chrome Web Store listing drafts, first runtime shell slices, popup structured-copy buckets through `Phase 392.4`, Settings core copy through `Phase 393.1`, Settings credential copy through `Phase 393.2.1`, Settings source/permission copy through `Phase 393.2.2`, Settings quota/progress item control copy through `Phase 438`, Provider Detail copy through `Phase 393.3.1`, provider-source display copy through `Phase 393.3.2`, typed warning diagnostic presentation through `Phase 398`, typed source-selection/fallback diagnostic presentation through `Phase 399`, and typed adapter-error diagnostic presentation through `Phase 400`, while remaining deeper runtime copy falls back to English where translation review has not occurred
