...
- non-`zh-CN` deep runtime structured copy outside popup, Settings, and Provider Detail structured-copy buckets that currently falls back to English until reviewed translations are added
...
1. provider-source wrapper structured copy for all 14 runtime locales in `Phase 393.3.2`
2. typed diagnostic presentation follow-up if the Settings/provider-detail child phases stay small enough
3. operator-workspace and store-helper follow-up only after the end-user surfaces are stable
...
- `zh_TW`, `ja`, `ko`, `es_419`, `pt_BR`, `fr`, `de`, `it`, `ru`, `ar`, `hi`, and `id` currently cover Chrome manifest surfaces, Chrome Web Store listing drafts, first runtime shell slices, popup structured-copy buckets through `Phase 392.4`, Settings core copy through `Phase 393.1`, Settings credential copy through `Phase 393.2.1`, Settings source/permission copy through `Phase 393.2.2`, and Provider Detail copy through `Phase 393.3.1`, while remaining deeper runtime copy falls back to English where translation review has not occurred
