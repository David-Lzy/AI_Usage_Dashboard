...
- non-`zh-CN` deep runtime structured copy outside popup, Settings, Provider Detail, and provider-source display structured-copy buckets that currently falls back to English until reviewed translations are added
- typed adapter-error diagnostic presentation beyond the current `zh-CN` branch; typed warning diagnostic presentation has explicit 14-locale coverage after `Phase 398`, and typed source-selection/fallback diagnostic presentation has explicit 14-locale coverage after `Phase 399`
...
1. adapter-error diagnostic presentation follow-up after the `Phase 398` warning slice and `Phase 399` source slice
2. operator-workspace and store-helper follow-up only after the end-user surfaces are stable
...
- `zh_TW`, `ja`, `ko`, `es_419`, `pt_BR`, `fr`, `de`, `it`, `ru`, `ar`, `hi`, and `id` currently cover Chrome manifest surfaces, Chrome Web Store listing drafts, first runtime shell slices, popup structured-copy buckets through `Phase 392.4`, Settings core copy through `Phase 393.1`, Settings credential copy through `Phase 393.2.1`, Settings source/permission copy through `Phase 393.2.2`, Provider Detail copy through `Phase 393.3.1`, provider-source display copy through `Phase 393.3.2`, typed warning diagnostic presentation through `Phase 398`, and typed source-selection/fallback diagnostic presentation through `Phase 399`, while remaining deeper runtime copy falls back to English where translation review has not occurred
