...
- non-`zh-CN` deep runtime structured copy outside popup and Settings structured-copy buckets that currently falls back to English until reviewed translations are added
...
1. Provider Detail and provider-source wrapper structured copy for all 14 runtime locales in `Phase 393.3`
...
- `zh_TW`, `ja`, `ko`, `es_419`, `pt_BR`, `fr`, `de`, `it`, `ru`, `ar`, `hi`, and `id` currently cover Chrome manifest surfaces, Chrome Web Store listing drafts, first runtime shell slices, popup structured-copy buckets through `Phase 392.4`, Settings core copy through `Phase 393.1`, Settings credential copy through `Phase 393.2.1`, and Settings source/permission copy through `Phase 393.2.2`, while remaining deeper runtime copy falls back to English where translation review has not occurred
