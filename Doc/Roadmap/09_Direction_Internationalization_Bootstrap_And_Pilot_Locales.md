...
- `zh-TW`, `ja`, `ko`, `es-419`, `pt-BR`, and `fr` now have first runtime shell pilots for dashboard, popup, Settings, common actions, and theme-toggle labels; deeper structured runtime copy still falls back to English until reviewed translations are added
...
45. deeper runtime catalog translation review for non-English fallback locales - future work
46. deeper diagnostic-body localization - deferred until a separate product need exists and the raw evidence compatibility boundary remains intact
