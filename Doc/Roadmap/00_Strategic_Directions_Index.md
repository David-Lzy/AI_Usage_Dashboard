...
- the numbered phase queue is completed through `Phase 505`; no numbered phase is currently queued after `Phase 505`; `0.1.0-rc.23` is the latest packaged follow-up candidate through `Phase 502`, current source is ahead through the source-level provider-entry migration, long-lived-profile migration fix, and popup provider-parity fix, and RC13 remains the submitted store-review boundary
...
1. [Direction 10 - Toolbar Competitive Fit And Store Readiness](./10_Direction_Toolbar_Competitive_Fit_And_Store_Readiness.md)
   Why first now:
   `0.1.0-rc.23` is now packaged as the latest follow-up candidate through `Phase 502`, current source is ahead through `Phase 505`, no numbered phase is currently queued after `Phase 505`, the public-readiness screenshot archive and six-locale listing handoff are complete, and the trimmed transparent icon refresh plus Claude Team usage-page path are already in the submitted RC13 boundary.
