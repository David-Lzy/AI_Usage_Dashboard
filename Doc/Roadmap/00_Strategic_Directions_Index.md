...
- the numbered phase queue is completed through `Phase 500`; `Phase 501` is active for provider setup/display model closeout; `0.1.0-rc.22` is the latest packaged follow-up candidate through `Phase 487`, current source is ahead through AGPL-3.0 open-source licensing, digital fingerprint embedding, popup provider-visibility fix, public repository readiness, six-locale store listing handoff, refreshed public-readiness screenshot evidence, the provider setup/display product contract, Quick Setup source-mode cards, shared provider display eligibility, visible-provider surface order alignment, and quota item eligibility alignment, and RC13 remains the submitted store-review boundary
...
1. [Direction 10 - Toolbar Competitive Fit And Store Readiness](./10_Direction_Toolbar_Competitive_Fit_And_Store_Readiness.md)
   Why first now:
   `0.1.0-rc.22` is now packaged as the latest follow-up candidate through `Phase 487`, current source is complete through `Phase 500`, `Phase 501` now handles provider setup/display closeout after the store handoff, the public-readiness screenshot archive and six-locale listing handoff are complete, and the trimmed transparent icon refresh plus Claude Team usage-page path are already in the submitted RC13 boundary.
