...
- the numbered phase queue is now completed through `Phase 175`
...
- the next repo-owned engineering slice now moves to locale-aware durations plus compact-width and RTL hardening under Direction 09, captured in `09_2_Runtime_I18n_Bootstrap_And_Pilot_Locales_TODOs.md`
...
- the broader `Doc/` tree is not "fully done" even though the numbered phase queue is completed through `Phase 175`, because roadmap, request, archive-index, package-record, and reference docs remain living or maintained by design
- the numbered phase queue is now completed through `Phase 175`, and the repo now has one explicit documentation-class vocabulary plus one explicit freshness-label vocabulary plus one lightweight executable consistency check that also covers generated package READMEs, the full roadmap set, and the current convention-only boundary
...
- the current extension now has manifest localization plus a broader runtime pilot slice in `en` and `zh_CN`, covering popup/dashboard shells, popup explanatory copy, the first settings-shell rollout, deeper settings helper copy, provider-detail shell/static copy, and locale-aware generated values; raw provider source-truth detail strings, localized durations, and operator workspaces still remain outside the shipped pilot
...
1. [Direction 09 - Internationalization Bootstrap And Pilot Locales](./09_Direction_Internationalization_Bootstrap_And_Pilot_Locales.md)
   Why first now:
   manifest localization, the first runtime shell slice, locale-aware value formatting, the first settings-shell pilot, the popup/provider-detail pilot rollout, and the deeper settings-helper rollout are now shipped, so the next fully repo-owned engineering work is locale-aware durations plus compact-width and RTL hardening for the `en` plus `zh_CN` pilot.
