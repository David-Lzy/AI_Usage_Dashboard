...
const expectedPackageVersion = "0.1.0-rc.7";
const expectedManifestVersion = "0.1.0.7";
const archiveRelativePath =
  "release/ai-usage-dashboard-0.1.0-rc.7.zip";
const artifactDir = path.join(
  projectRoot,
  "tmp",
  "phase288-source-chip-row-rc7-review",
);
...
async function verifyPackageScript() {
  ...
  assert(
    packageJson.scripts["phase288:review"] ===
      "./scripts/with-preferred-node.sh node ./scripts/phase288-source-chip-row-rc7-review.mjs",
    "package.json is missing the expected phase288:review script.",
  );
...
async function verifyCssMarkers() {
  const css = await readProjectFile("src/sidepanel/theme/settings-source-cards.css");

  verifyMarkers(css, "src/sidepanel/theme/settings-source-cards.css", [
    ".source-card__chips {",
    "display: flex",
    "flex-wrap: wrap",
    "justify-content: flex-end",
    ".source-card__chips .meta-chip",
    "white-space: nowrap",
...
async function verifyDocsMarkers() {
  const expectations = [
    ...
    },
    {
      ...
      markers: [
        "288_Phase_Source_Chip_Row_And_RC7_Packaging.md",
...
async function runReview() {
  ...
  const reportPath = path.join(artifactDir, "source-chip-row-rc7-review.json");

  await writeFile(reportPath, JSON.stringify(report, null, 2), "utf8");

  console.log("Phase 288 source-chip row RC7 review passed.");
