...
const expectedPackageVersion = "0.1.0-rc.5";
const expectedManifestVersion = "0.1.0.5";
const archiveRelativePath =
  "release/ai-usage-dashboard-0.1.0-rc.5.zip";
const artifactDir = path.join(projectRoot, "tmp", "phase286-rc5-release-package-review");
...
async function verifyPackageScript() {
  ...
  assert(
    packageJson.scripts["phase286:review"] ===
      "./scripts/with-preferred-node.sh node ./scripts/phase286-rc5-release-package-review.mjs",
    "package.json is missing the expected phase286:review script.",
  );
  ...
  const expectations = [
    {
      ...
      markers: [
        ...
        archiveRelativePath,
        "Phase 285 post-rc4 smoke polish",
      ],
    },
    {
    ...
    },
    {
      ...
      markers: [
        "286_Phase_RC5_Release_Packaging.md",
...
async function runReview() {
  ...
  const reportPath = path.join(artifactDir, "rc5-release-package-review.json");

  await writeFile(reportPath, JSON.stringify(report, null, 2), "utf8");

  console.log("Phase 286 RC5 release package review passed.");
