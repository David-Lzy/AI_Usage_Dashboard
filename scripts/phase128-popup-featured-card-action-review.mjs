...
const artifactDir = path.join(
  projectRoot,
  "tmp",
  "phase128-popup-featured-card-action-review",
);
...
const scenarios = [
  {
    ...
    expectedActionLabel: "Open settings",
  },
  {
    slug: "needs-review",
    expectedStatusLabel: "Needs review",
    expectedActionLabel: "Review detail",
  },
  {
    slug: "policy-only",
    expectedStatusLabel: "Contract-only",
    expectedActionLabel: "Open dashboard",
  },
  {
    slug: "healthy",
    expectedStatusLabel: "Healthy",
    expectedActionLabel: "Open detail",
  },
];
...
async function reviewScenarioWidth(context, scenario, width) {
  ...
  console.log(`phase128: ${scenario.slug}@${width}`);
  await seedScenario(page, scenario.slug);
  await page.waitForSelector('[data-popup-featured-action="true"]');
  ...
  assert(
    snapshot.featuredAction === scenario.expectedActionLabel,
    `${scenario.slug}@${width}: expected featured action "${scenario.expectedActionLabel}", got "${snapshot.featuredAction}"`,
  );
...
async function main() {
  ...
  const resultsPath = path.join(artifactDir, "phase128-results.json");
  await writeFile(resultsPath, `${JSON.stringify(results, null, 2)}\n`);

  console.log(`phase128: wrote ${path.relative(projectRoot, resultsPath)}`);
}
