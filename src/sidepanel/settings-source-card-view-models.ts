...
function buildSessionTrackModel(
  ...
  const fields = [
    buildField(
      labels.primary.route,
      routeHint ??
        labels.routeFallback ??
        DEFAULT_SETTINGS_SOURCE_CARD_LABELS.routeFallback,
    ),
    buildField(
      labels.primary.availability,
      sourceDisplay.sessionPageAvailabilitySummary,
    ),
    buildField(
      labels.primary.graduationGate,
...
export function buildSettingsSourceCardModel(
  ...
  const fallbackValue =
    sourceDisplay.fallbackPlans.length > 0
      ? sourceDisplay.fallbackPlans
          .map(
            (plan) =>
              (labels.sourceKindLabels ??
                DEFAULT_SETTINGS_SOURCE_CARD_LABELS.sourceKindLabels!)[plan.kind] ??
              plan.kind,
          )
          .join(" · ")
  ...
  const primaryFields = [
    ...
    buildField(
      labels.primary.availabilitySummary,
      sourceDisplay.availabilitySummary,
    ),
  ...
  const diagnosticGroups = [
    buildDiagnosticGroup(
      ...
      [
        ...
        buildField(
          labels.primary.graduationGate,
          sourceDisplay.currentGraduationGateLabel,
        ),
      ],
      [
