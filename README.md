...
- source status: `rc.23` package bytes are aligned through `Phase 502`; current source is ahead through `Phase 505`, and no numbered phase is currently queued after `Phase 505`. The earlier `rc.13` milestone remains the submitted Chrome Web Store review boundary until a human resubmission replaces it.
...
1. keep `0.1.0-rc.13` as the current submitted Chrome Web Store review boundary; `0.1.0-rc.23` is the latest packaged follow-up candidate, while source-level provider-entry work, the long-lived-profile migration fix, and the popup provider-parity fix after RC23 need a new package before resubmission
...
3. no numbered phase is currently queued after [Phase 505](./Doc/TODOs/Archive/by-phase/500-599/505_Phase_Popup_Provider_Parity_And_Quota_Warning_Fix.md)
...
- `rc.23` packages the public-source readiness and provider setup/display cleanup boundary through `Phase 502`; current source is ahead through `Phase 505` and is not yet packaged.
