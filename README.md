...
[![License: AGPL-3.0](https://img.shields.io/badge/License-AGPL--3.0-blue.svg)](https://github.com/David-Lzy/AI_Usage_Dashboard/blob/main/LICENSE)
...
Anyone who distributes this software or a modified version of it must make the complete source code available under the same license.
...
- source status: `rc.22` package bytes are aligned through `Phase 487`; current source is ahead through `Phase 499` with AGPL/build-fingerprint metadata, popup provider-visibility fix, public repository readiness docs, six-locale store listing handoff, refreshed public-readiness screenshot evidence, the provider setup/display product contract, Quick Setup source-mode cards, shared provider display eligibility, and visible-provider surface order alignment. `Phase 500` through `Phase 501` are now queued for provider setup/display model cleanup. The earlier `rc.13` milestone remains the submitted Chrome Web Store review boundary until a human resubmission replaces it.
...
3. run the remaining provider setup/display model cleanup queue from [Phase 500](./Doc/TODOs/500_Phase_Quota_Item_Eligibility_And_Popup_Order_QA.md) through [Phase 501](./Doc/TODOs/501_Phase_Provider_Setup_Display_Model_Closeout.md), using the completed [Phase 496 product contract](./Doc/Product/Provider_Setup_Display_Product_Contract.md), [Phase 497 Quick Setup source-mode cards](./Doc/TODOs/Archive/by-phase/400-499/497_Phase_Quick_Setup_Source_Mode_Cards.md), [Phase 498 display eligibility model](./Doc/TODOs/Archive/by-phase/400-499/498_Phase_Dashboard_Display_Eligibility_Model.md), and [Phase 499 surface order alignment](./Doc/TODOs/Archive/by-phase/400-499/499_Phase_Surface_Order_Eligibility_Alignment.md)
...
- Toolbar action icon preferences now live in `src/shared/toolbar-icon-preferences.ts`, `src/background/action-icon.ts`, and Settings preference controls, so users can keep the default icon, match a quota badge provider, choose a provider favicon, or upload a bounded custom image without changing Web Store or extension-management icons
...
- UI font-family preference normalization now lives in `src/shared/ui-font-family.ts`, and the selected safe local stack is applied through shared Material typography variables by `src/shared/theme.ts`
...
- Configuration backup and Chrome Sync helpers now live in `src/shared/configuration-backup.ts`, with Settings controls routed through the standard background message channel so JSON import/export and `chrome.storage.sync` save/restore preserve local-only credentials, page bindings, permissions, and runtime evidence
- Toolbar action badge preferences now support multiple selected badge sources plus a dedicated rotation alarm, while `match-badge` toolbar icons follow the same active rotated badge provider
- Toolbar action badge source selection now presents that multi-select state as one compact Material-style dropdown summary instead of an always-visible checkbox stack
- Toolbar action badge source selection and badge rotation interval now render as separate Settings grid controls with helper text behind Material-style info tooltips
...
- interaction-audit frame-result display now uses `src/sidepanel/interaction-audit-frame-result-presentation.ts` plus `interactionAudit.frameResults` localized copy, keeping typed display text separate from raw selector and preset diagnostics
...
- Settings quota item controls now live in `src/sidepanel/components/ProviderProgressItemPreferenceControls.tsx`, with 14-locale display copy in `src/shared/settings-progress-items-localized-copy.ts`, keeping per-surface progress item show/hide and reorder UI outside the larger Settings preferences section; Provider display shows every provider summary first and keeps each provider's quota-item detail collapsed until expanded
...
- Settings overview display-level help now uses `settings-overview__user-level-help`, so wide layouts place the helper beside the selector and narrow localized layouts stack it below without changing user-level behavior
- Settings appearance grids now use `settings-grid--balanced-settings`, quick setup/source field grids use count-aware balanced wrapping, and field-level tooltip triggers render in the field label row instead of beside the input frame
- Settings form label/accessory rendering now uses `src/sidepanel/components/FormFieldLabel.tsx` across Material select, editable number combobox, and action badge selection controls
- special/debug sidepanel routes now lazy-load from `src/sidepanel/special-route-app.tsx`, and popup/sidepanel runtime copy imports use focused copy modules instead of the compatibility `src/shared/localized-copy.ts` aggregator
- Settings CSS now consolidates shared label/info-row rules and source-card summary/session grid rules without changing visual spacing or responsive behavior
