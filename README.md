...
- source status: `rc.21` package bytes are aligned through `Phase 469`; no numbered phase is currently queued after `Phase 469`. The earlier `rc.13` milestone remains the submitted Chrome Web Store review boundary until a human resubmission replaces it.
...
1. keep `0.1.0-rc.13` as the current submitted Chrome Web Store review boundary, but use `0.1.0-rc.21` as the ready follow-up package if review feedback or an explicit resubmission decision asks for a newer build
2. use the [RC13 upload-candidate milestone](./Doc/Milestones/2026-05-11_RC13_Chrome_Web_Store_Upload_Candidate.md) as the truthful historical submission handoff, and use the [RC21 follow-up milestone](./Doc/Milestones/2026-05-15_RC21_Settings_Tooltip_Form_Control_Release_Candidate.md) as the current packaged-source reference
3. no numbered phase is currently queued after `Phase 469`; future work should start from a new scoped TODO rather than silently changing the RC21 package boundary
...
- `rc.21` packages the previous `rc.20` follow-up work plus `Phase 468` Settings tooltip and form-control polish, then closes the package boundary through `Phase 469`; no numbered phase is currently queued after `Phase 469`.
