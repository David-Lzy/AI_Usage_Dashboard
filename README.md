...
- source status: `rc.22` package bytes are aligned through `Phase 487`; current source is ahead through `Phase 500` with AGPL/build-fingerprint metadata, popup provider-visibility fix, public repository readiness docs, six-locale store listing handoff, refreshed public-readiness screenshot evidence, the provider setup/display product contract, Quick Setup source-mode cards, shared provider display eligibility, visible-provider surface order alignment, and quota item eligibility alignment. `Phase 501` is now active for provider setup/display closeout. The earlier `rc.13` milestone remains the submitted Chrome Web Store review boundary until a human resubmission replaces it.
