...
- the repo now also ships one broader runtime i18n pilot plus one locale-aware formatting slice:
  ...
  - adapter diagnostic reason-code planning now exists in [I18n_Adapter_Diagnostic_Reason_Code_Plan.md](./Doc/I18n_Adapter_Diagnostic_Reason_Code_Plan.md), so the next safe i18n runtime work is a type-only additive diagnostic model rather than translated diagnostic bodies
