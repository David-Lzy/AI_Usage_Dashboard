...
- the repo now also ships one broader runtime i18n pilot plus one locale-aware formatting slice:
  ...
  - adapter diagnostic reason-code planning now exists in [I18n_Adapter_Diagnostic_Reason_Code_Plan.md](./Doc/I18n_Adapter_Diagnostic_Reason_Code_Plan.md), the type-only additive diagnostic model now exists in provider types/helpers, and Cursor plus Codex source-selection/fallback plus credential/host-access diagnostics now populate typed metadata without changing rendered UI behavior or raw adapter strings
