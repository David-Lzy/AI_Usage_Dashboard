...
- source status: `rc.22` package bytes are aligned through `Phase 487`; current source is ahead through `Phase 496` with AGPL/build-fingerprint metadata, popup provider-visibility fix, public repository readiness docs, six-locale store listing handoff, refreshed public-readiness screenshot evidence, and the provider setup/display product contract. `Phase 497` through `Phase 501` are now queued for provider setup/display model cleanup. The earlier `rc.13` milestone remains the submitted Chrome Web Store review boundary until a human resubmission replaces it.
...
3. run the provider setup/display model cleanup queue from [Phase 497](./Doc/TODOs/497_Phase_Quick_Setup_Source_Mode_Cards.md) through [Phase 501](./Doc/TODOs/501_Phase_Provider_Setup_Display_Model_Closeout.md), using the completed [Phase 496 product contract](./Doc/Product/Provider_Setup_Display_Product_Contract.md) before changing provider ordering or Quick Setup behavior
