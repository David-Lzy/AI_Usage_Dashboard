...
- source status: `rc.21` package bytes are aligned through `Phase 469`; current source is ahead through `Phase 471` surface-switch close-target fix, and no numbered phase is currently queued after `Phase 471`. The earlier `rc.13` milestone remains the submitted Chrome Web Store review boundary until a human resubmission replaces it.
...
1. keep `0.1.0-rc.13` as the current submitted Chrome Web Store review boundary; `0.1.0-rc.21` remains the latest packaged follow-up candidate, but the current source is ahead by `Phase 471` and needs a fresh package before resubmission if this navigation change should ship
...
3. no numbered phase is currently queued after `Phase 471`; future work should start from a new scoped TODO rather than silently changing the RC21 package boundary
...
- current source is ahead of `rc.21` by `Phase 471` surface-switch close-target fix; package a fresh RC before using this change for a store resubmission.
