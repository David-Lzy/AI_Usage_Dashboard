...
- source status: `rc.22` package bytes are aligned through `Phase 487`; current source is ahead through `Phase 498` with AGPL/build-fingerprint metadata, popup provider-visibility fix, public repository readiness docs, six-locale store listing handoff, refreshed public-readiness screenshot evidence, the provider setup/display product contract, Quick Setup source-mode cards, and shared provider display eligibility. `Phase 499` through `Phase 501` are now queued for provider setup/display model cleanup. The earlier `rc.13` milestone remains the submitted Chrome Web Store review boundary until a human resubmission replaces it.
