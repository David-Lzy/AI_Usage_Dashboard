...
3. no numbered phase is currently queued after `Phase 369`; if continuing unattended, review the project again and create a new small TODO slice before changing behavior
...
- runtime locale support now uses the 14-locale registry in `src/shared/i18n.ts`, Settings language options are generated from that registry, Arabic resolves `rtl`, and `npm run i18n:check` guards the 14 Chrome manifest `_locales` catalogs added after `rc.15`
