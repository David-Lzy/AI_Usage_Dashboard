...
- source status: `rc.22` package bytes are aligned through `Phase 487`; current source is ahead through `Phase 495` with AGPL/build-fingerprint metadata, popup provider-visibility fix, public repository readiness docs, six-locale store listing handoff, and refreshed public-readiness screenshot evidence. `Phase 496` through `Phase 501` are now queued for provider setup/display model cleanup. The earlier `rc.13` milestone remains the submitted Chrome Web Store review boundary until a human resubmission replaces it.
...
3. run the provider setup/display model cleanup queue from [Phase 496](./Doc/TODOs/496_Phase_Provider_Setup_Display_Product_Contract.md) through [Phase 501](./Doc/TODOs/501_Phase_Provider_Setup_Display_Model_Closeout.md) before changing provider ordering or Quick Setup behavior
